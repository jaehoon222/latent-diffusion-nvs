# autoencoder 학습 시키려면 imagenet 설치 필요함
# 걍 오토인코더는 pretrain 된거 사용
...
class ImageNetTrain(ImageNetBase):
    ...
    def __init__(self, process_images=True, data_root="data/", **kwargs):
    ...
    def _prepare(self):
        if self.data_root:
            ...
            raise ValueError(
                "data_root must be specified with the path to the dataset."
            )
        self.datadir = self.root  # datadir이 그대로 루트가 됩니다.
        self.txt_filelist = os.path.join(self.root, "filelist.txt")
        self.expected_length = None  # expected_length는 필요 없으므로 None으로 설정
        self.random_crop = retrieve(
            self.config, "ImageNetTrain/random_crop", default=True
        )

        # filelist.txt 파일이 없으면 생성
        if not os.path.exists(self.txt_filelist):
            print("Generating filelist.txt in {}".format(self.root))
...
class ImageNetSR(Dataset):
    ...
    def get_base(self):
        ...
        # 데이터셋 로딩 경로를 data_root를 이용하여 설정
        dataset_path = os.path.join(self.data_root, "imagenet_train_hr_indices.p")
